...
export interface ISprintStore {
  ...
  sprintModalActiva: ISprint | null;
  setsprintActiva: (sprintActiva: ISprint | null) => void;
  setSprintModalActiva: (sprintActiva: ISprint | null) => void;
...
export const sprintStore = create<ISprintStore>((set) => ({
  ...
  eliminarUnasprint: (idsprint) =>
    set((state) => {
      const arreglosprints = state.sprints.filter(
  ...
  setSprintModalActiva: (sprintActivaIn) =>
    set(() => ({ sprintModalActiva: sprintActivaIn })),
}));
